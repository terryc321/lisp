...
;; (define (eval-timing exp env cont)
;;   (let ((timed-part (car (cdr exp)))
;; 	(start-clock (real-time-clock))
;; 	(stop-clock 0)
;; 	(time-taken 0))
;;     (meta-apply 'base-eval timed-part env
;; 		(lambda (p)
;; 		  (set! stop-clock (real-time-clock))
;; 		  (set! time-taken (- stop-clock start-clock))
;; 		  (display "timings : ")
;; 		  (display (list 'start start-clock 'stop stop-clock 'time-taken time-taken))
;; 		  (newline)
;; 		  (cont p)))))
...
(define init-env (list (list
			;;  (cons 'with-timings           with-timings)
  (cons 'real-time-clock   real-time-clock)			
  (cons 'car			car)
  (cons 'cdr			cdr)
  (cons 'cons			cons)
  (cons 'list			list)
