...
;; this is defined again later on. 
;; (define tower
;;   (letrec
;;       ((loop
;;         (lambda (n)
...
(define denotation
  (lambda (e)
    (cond
     ;;  if not symbol or pair , then just constant
     ((not (or (pair? e) (symbol? e))) (denotation-of-value e))
     ;; is it a symbol ?
     ((symbol? e) (denotation-of-identifier e))
     ;; is it a lambda abstraction
...
;; the initial definition has some problems , immediately redefined in the paper.
;; (define make-reifier
;;   (curry (bf e r k)
;;          (shift-up (bf (list e
;;                              (U->BF
...
(define boot-global-env
  (let ((id->F-cell (lambda (x) (make-cell (id->BF x)))))
    (lambda ()
      (let ((initnames
             (append
              ;; i think this means #f #t 'wrong 'mean , rather than 'nil 't 
              (list #f
                    #t
                    'wrong
                    'meaning)
...
;; atom is either
(define atom? (lambda (x) (or (symbol? x)
			      (number? x))))
...
(define R-E-P
  (lambda (prompt)
    (prompt&read (lambda (v)
                   (print&prompt prompt v)
                   (if (eof-object? v)
                       (lambda (mk) '---done---)
                       (((denotation v) global-env) R-E-P))))))
...
;;
(boot-tower)
(brown-load "/home/terry/lisp/brown-scheme/prog.brn")
