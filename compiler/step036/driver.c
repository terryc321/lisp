...
void scheme_gc_root(char *ptr){
  ...
  if (n2 == 0 // fixnum
    ...
    // not a pointer - we can ignore it
    return;
  }
  ...
  // untag pointer address
  unsigned int n3 = (n & (! 7));

  // check pointer points to an object
  // look at isobject_from bitmap
  if (! bitmap_get_bit(bitmap_isobject_from , n3)){
    // not pointing at object
    return ;
  }

  // n3 points into the from heap , its a valid pointer to start of some object.
  // n2 tells us the type of object we are looking at
